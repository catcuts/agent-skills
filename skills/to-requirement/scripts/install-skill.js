...
// 技能名称
const skillName = 'to-requirement';
...
function cleanOldInstallations() {
    ...
    // 1. 清理规范副本目录（.agents/skills/to-requirement）
...
try {
    log(`开始安装 To-Requirement Skill...`, 'info');
    ...
    log('\n安装成功!', 'success');
    log(
        `Skill 已安装到: ${isGlobal ? '~/.claude/skills/to-requirement' : '.claude/skills/to-requirement'}`,
        'info'
    );

    // 显示使用指南
    console.log('\n使用示例:');
    console.log('  - 分析文章需求: 使用 /to-requirement 技能');
    console.log('  - 提供文章路径: 将文章文件路径传递给技能');
    console.log('\n更多信息请查看: https://github.com/catcuts/agent-skills');
} catch (error) {
    handleError(error);
}
